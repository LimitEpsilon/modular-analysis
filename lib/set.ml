...
type 'e t = Empty | Node of {l : 'e t; v : 'e; r : 'e t; h : int}
...
type 'e split_bis = Found | NotFound of 'e t * (unit -> 'e t)
...
let rec filter p = function
  ...
  | Node {l; v; r} as t ->
    ...
    let pv = p v in
    let r' = filter p r in
    if pv then if l == l' && r == r' then t else join l' v r' else concat l' r'
...
let rec map f = function
  ...
  | Node {l; v; r} as t ->
    ...
    let v' = f v in
    let r' = map f r in
    if l == l' && v == v' && r == r' then t else try_join l' v' r'
...
let rec filter_map f = function
  ...
  | Node {l; v; r} as t -> (
    ...
    match v' with
    | Some v' -> if l == l' && v == v' && r == r' then t else try_join l' v' r'
    | None -> try_concat l' r')
...
let to_seq_from low s =
  let rec aux low s c =
    ...
    | Empty -> c
    | Node {l; r; v; _} -> (
      match Stdlib.compare v low with
      | 0 -> More (v, r, c)
