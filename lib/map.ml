...
type ('k, 'a) t =
  | Empty
  | Node of {l : ('k, 'a) t; v : 'k; d : 'a; r : ('k, 'a) t; h : int}
...
let rec find_first f = function
  | Empty -> raise Not_found
  | Node {l; v; d; r} -> if f v then find_first_aux v d f l else find_first f r
...
let rec update x f = function
  ...
  | Node {l; v; d; r; h} as m ->
    ...
    if c = 0 then
      match f (Some d) with
      | None -> merge l r
      | Some data -> if d == data then m else Node {l; v = x; d = data; r; h}
    else if c < 0 then
