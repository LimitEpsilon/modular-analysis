...
\begin{proof}[Sketch]
  ...
  If we have the lemmas, then for all $\A\rho$ such that $(e,\A{s})\A\semarrow^*\A\rho$, $\lfloor\A\rho\rfloor\subseteq f(\rho)\subseteq f(\ell)$, thus all signatures in $\Abs{\sembracket{e}}\Abs{S}$ can be bound by $X\triangleq\{f(e,\A{s})|(\A{s},\_)\in\Abs{S}\}$, which is finite.
...
\section{Part III: Formalizing Linking in the Semantics with Memory}
