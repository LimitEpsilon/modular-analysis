...
In our case, $D\triangleq\wp(\Sigma)$ when $\Sigma\triangleq\semarrow\uplus\Right$ and $\wp(S)$ is the powerset of $S$.
...
\[D=\wp(\Sigma)\galois{\alpha}{\gamma}\Abs{D}\]
...
That is, (1) $t\not\in\mathsf{supp}(C,\mem)$, and (2) $\tick(t)\not\in\mathsf{supp}(C,\mem)\wedge\tick(t)\neq t$ must be guaranteed, when $\mathsf{supp}(C,\mem)$ is the set of timestamps reachable from $(C,\mem)$.
...
Then the first criteria, that $t$ must be fresh in $C,\mem$, is formalized by demanding that $C<t$ and $\mem<t$, when:
\[
  \begin{array}{cc}
    C < t\triangleq
    \begin{cases}
  ...
  \end{array}
\]
. The second criteria, that $\tick(t)$ must also be fresh and must not equal $t$, is formalized by demanding that:
\[t < \tick(t)\]
for all $t$.
...
\[
  \State\triangleq\{(C,\mem,t)|C<t\wedge\mem<t\}\qquad
  \Result\triangleq\{(V,\mem,t)|V<t\wedge\mem<t\}
\]
as the set of \emph{valid} states that enable $\tick$ to behave nicely.
It is almost trivial that the set $\Left\times\Result$, when $\Left\triangleq\Expr\times\State$ and $\Right\triangleq\Left\uplus\Result$, is \emph{closed} under the inductive definition of $\semarrow$.
That is,
...
That is, when we let $D\triangleq\wp(\Sigma)$ where $\Sigma\triangleq\Right\uplus\semarrow$,
\begin{definition}[Transfer function]
  Given $A\in D$, define
  \[
    \mathsf{Step}(A)\triangleq
...
Then if we define $\Abs{D}\triangleq\wp(\A\Sigma)$, when $\A\Sigma\triangleq\A\Right\uplus\A\semarrow$, we can establish a Galois connection between $D$ and $\Abs{D}$.
...
\begin{clm}[Galois Connection]
  $\wp(\Sigma)=D\galois{\alpha}{\gamma}\Abs{D}=\wp(\A\Sigma)$. That is, $\forall A\in D,\Abs{A}\in\Abs{D}:\alpha(A)\subseteq\Abs{A}\Leftrightarrow A\subseteq\gamma(\Abs{A})$.
\end{clm}
\begin{proof}[Sketch]
...
\begin{proof}
  ...
  Thus, $\alpha\circ\Step\circ\gamma\subseteq\Abs\Step\circ\alpha\circ\gamma$, since $\gamma$ is monotonic.
...
\begin{proof}[Sketch]
  ...
  To elaborate, let this function be called $f:\A\Right\rightarrow\wp(\Sig)$.
  ...
  If we have the lemmas, then for all $\A\rho$ such that $(e,\A{s})\A\semarrow^*\A\rho$, $\lfloor\A\rho\rfloor\subseteq f(\A\rho)\subseteq f(e,\A{s})$, thus all signatures in $\Abs{\sembracket{e}}\Abs{S}$ can be bound by $X\triangleq\{f(e,\A{s})|(\A{s},\_)\in\Abs{S}\}$, which is finite.
...
Only the definition of $\inject{C_1}{\mem_2}$ in Figure \ref{fig:concinject} has to be adapted to account for the fact that $\mem_2(t)$ is now a \emph{set} of closures and thus injection of $C_1$ must be mapped over all elements.
...
\begin{proof}[Sketch]
  ...
  \[
    \begin{array}{l}
      ...
      \exists\rho',\phi,\phi^{-1}:\ell'\semarrow\rho'\wedge(\rho,\phi)\cong(\rho',\phi^{-1})\wedge\varphi(n)|_{n<t}=\phi(n)|_{n<t} \\
