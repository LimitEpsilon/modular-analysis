...
In our case, $D\triangleq\wp(\semarrow\uplus\Right )$.
...
We can elucidate what $\sembracket{e}S$ in terms of the operational semantics.
\begin{clm}[Collecting Semantics in Another Form]
  For all $A\subseteq\Left$,
  \[\lfp(\lambda X.\Step(X)\cup A)=\{\ell'\semarrow\rho,\ell',\rho|\exists\ell\in A:\ell\semarrow^*\ell'\}\]
  when $\semarrow^*$ is the reflexive transitive closure of $\semarrow$.
\end{clm}
\begin{proof}[Sketch]
  We first prove that
  \[\forall X\subseteq D,\ell\in\Left,\rho\in\Right:\Step(X)\subseteq X\wedge\ell\in X\wedge\ell\semarrow\rho\Rightarrow\ell\in\rho,\rho\in X\]
  . This is a straightforward proof by induction on the derivation of $\semarrow$.
  Then we prove that
  \[\forall X\subseteq D,\ell\in\Left,\rho\in\Right:\Step(X)\subseteq X\wedge\ell\in X\wedge\ell\semarrow^*\rho\Rightarrow\rho\in X\]
  . This is a straightforward proof by induction on the derivation of $\semarrow^*$.

  Then we have that a fixed point of $\lambda X.\Step(X)\cup A$ must contain the right hand side.
  Since the right hand side \emph{is} a fixed point of $\lambda X.\Step(X)\cup A$, it must be the least fixed point.
\end{proof}
...
\begin{clm}[Injection Preserves Evaluation]
  ...
  \[(\ell,\rho)\in D\Rightarrow(\inject{C}{\ell},\inject{C}{\rho})\in D\]
\end{clm}
\begin{proof}[Sketch]
...
\begin{definition}[Injection]
  ...
  Then for $S\subseteq\Ctx$ and $A\subseteq D$, define:
...
\begin{definition}[Semantic Linking]
  For $S\subseteq\Ctx$ and $A\subseteq D$, define:
...
\begin{proof}
  ...
  . $\wp(D)$ is a complete lattice, so by the Knaster-Tarski theorem, $X_A$ is the least of all pre-fixed points.
...
We require a CPO $\Abs{D}$ that is Galois connected with $\wp(D)$ by abstraction $\alpha$ and concretization $\gamma$:
\[\wp(D)\galois{\alpha}{\gamma}\Abs{D}\]
and semantic operators $\Abs\Step$ and $\Abs\rhd$ that satisfies:
\[\Step\circ\gamma\subseteq\gamma\circ\Abs\Step\qquad S_1\rhd S_2\subseteq\gamma(\alpha(S_1)\Abs\rhd\alpha(S_2))\]
and a linking operator $\Abs\semlink$ that satisfies:
\[\Abs{\sembracket{e}}(\Abs{S}_1\Abs\rhd\Abs{S}_1)=\Abs{S}_1\Abs\semlink\Abs{\sembracket{e}}\Abs{S}_2\]
. Then we can soundly approximate fractional specifications by:
\begin{align*}
  \sembracket{e}(S_1\rhd S_2) & \subseteq\sembracket{e}\gamma(\alpha(S_1)\Abs\rhd\alpha(S_2))         & (\because\rhd\subseteq\gamma\circ\Abs\rhd\circ\alpha)                                                         \\
                              & \subseteq\gamma(\Abs{\sembracket{e}}(\alpha(S_1)\Abs\rhd\alpha(S_2))) & (\because\Step\circ\gamma\subseteq\gamma\circ\Abs\Step)                                                       \\
                              & =\gamma(\alpha(S_1)\Abs\semlink\Abs{\sembracket{e}}\alpha(S_2))       & (\because\Abs{\sembracket{e}}(\Abs{S}_1\Abs\rhd\Abs{S}_1)=\Abs{S}_1\Abs\semlink\Abs{\sembracket{e}}\Abs{S}_2)
\end{align*}
