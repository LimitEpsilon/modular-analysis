...
\begin{figure}[htb]
  ...
  \begin{tabular}{rrcll}
    ...
    Expression  & $e$        & $\rightarrow$ & $()$ $\vbar$ $x$ $\vbar$ $\lambda x.e$ $\vbar$ $e$ $e$ & $\lambda$-calculus with unit \\
                ...
                &            & $\vbar$       & $\Lete$ $x$ $e$ $e$                                    & binding expression           \\
                &            & $\vbar$       & $\Letm$ $\modid$ $e$ $e$                               & binding module               \\
  \end{tabular}
...
\begin{figure}[h!]
  \footnotesize
  \begin{flushright}
    \fbox{$\Gamma\vdash e:\tau$ or $\Gamma\vdash e:\Gamma$}
  \end{flushright}
  \centering
  \vspace{0pt} % -0.75em}
  \[
...
\begin{clm}[Type Safety]
  ...
  Likewise, if $\bullet\vdash e:\Gamma$ for some $\Gamma$, then there exists some $\ctx'\in\Ctx$ such that $(e,\ctx)\Downarrow\ctx'$.
\end{clm}
\begin{proof}[Proof sketch]
  ...
  \begin{tabular}{rclr}
    ...
    $\Gamma\vDash e:\Gamma'$                  & $\triangleq$ & $\forall\ctx\in\CtxRel{\Gamma}:(e,\ctx)\in\CtxRel{\Gamma'}$
  \end{tabular}
...
\begin{flushright}\fbox{$V(\Gamma,e,\alpha)=U$ or $(\Gamma,U)$ or $\bot$}\end{flushright}
\hspace{-2.5em}
...
\begin{figure}[htb]
  ...
  \begin{tabular}{rrcll}
    ...
    Expression  & $e$ & $\rightarrow$ & $x$ $\vbar$ $\lambda x.e$ $\vbar$ $e$ $e$ & $\lambda$-calculus \\
                &     & $\vbar$       & $\link{e}{e}$                             & linked expression  \\
                &     & $\vbar$       & $\varepsilon$                             & empty module       \\
                &     & $\vbar$       & $\Lete$ $x$ $e$ $e$                       & exported binding
  \end{tabular}
...
\begin{align*}
  ...
  ((x,\tau)\cons\_)(x)    & \triangleq\tau                        \\
  ((x',\_)\cons\Gamma)(x) & \triangleq\Gamma(x) \qquad (x'\neq x) \\
  (\alpha.p)(x)           & \triangleq\alpha.px                   \\
  ([].p)(x)               & \triangleq[].px
\end{align*}
...
First, for $\external\in\TyEnv$, define the access operation $\external.p$ (which may fail):
\begin{align*}
  \external.\epsilon & \triangleq\external &  &  & \external.px & \triangleq(\external.p)(x)
\end{align*}
and define the injection operations $\Gamma[\external]$ and $\tau[\external]$:
\begin{align*}
  (\bullet)[\external]  & \triangleq\bullet  &  &  & ((x,\tau)\cons\Gamma)[\external] & \triangleq(x,\tau[\external])\cons\Gamma[\external] \\
...
\begin{align*}
  ...
  S(\alpha.p) & \triangleq\alpha.p & \text{when }\alpha\not\in dom(S)                                &  & S(\alpha.p)                & \triangleq \Gamma.p                   & \text{when }\alpha\mapsto\Gamma\in S \\
  S(\alpha.p) & \triangleq\bot     & \text{when }\alpha\mapsto\tau\in S\text{ and }\tau\not\in\TyEnv &  & S([].p)                    & \triangleq [].p
\end{align*}
Define:
\begin{align*}
  (S,\external)\vDash U\triangleq & \forall(\tau_1\A{=}\tau_2)\in U:(S\tau_1)[\external]= (S\tau_2)[\external]    \\
...
\end{align*}
where subtyping between type variables are defined as...(Work in Progress)
...
\begin{align*}
  \bot[\external] & \triangleq\bot                                                                                                                                                                                  \\
  U[\external]    & \triangleq\{\tau_1[\external]\A{=}\tau_2[\external]|(\tau_1\A{=}\tau_2)\in U\}\cup\{\tau_1[\external]\A{\le}\tau_2[\external]|(\tau_1\A{\le}\tau_2)\in U\} & \text{when all injections succeed} \\
  U[\external]    & \triangleq\bot                                                                                                                                             & \text{when injection fails}
\end{align*}
Then we can prove:
\begin{clm}[Advance]
