...
Proof.
  ...
  - des_hyp; des; clarify;
    match goal with
    | _ : Some ?v = read _ _ |- _ =>
      exists (trans_v α v); s;
      rewrite trans_m_aread
    end.
    all:split; try exists t; eexists; eauto.
    all:exploit IHp; eauto.
Qed.
...
Proof.
  ...
  - des_goal.
    ...
    rewrite trans_C_inject. ss.
    destruct (SPECα t tx). rrw.
    rewrite inject_assoc. ss. split. eauto.
    ii. apply PRE0.
    destruct p; ss;
    first [rewrite inject_addr_x in *
    | rewrite inject_ctx_M in *];
    repeat des_hyp; des; clarify;
    rewrite eqb_ID_eq in *; clarify.
    rw. eauto.
    (* not shadowed *)
    ...
    try rewrite trans_C_inject; s;
    try rewrite inject_assoc; s.
    ...
    rewrite trans_C_inject in *. ss.
    rewrite inject_assoc in *. ss.
    split. exists (dy_binde x (inv_α (t, tx)) Cin'). eauto.
    split. rewrite trans_C_inject in *. ss.
    ...
    (* main proofs *)
    + ii. repeat des_hyp; des; clarify.
      repeat rewrite trans_C_inject in *.
      repeat rewrite inject_assoc in *. ss.
      split. eauto.
  ...
  - des_goal.
    ...
    | |- context [iso] =>
      ...
      | _ =>
        ...
        first [solve [rewrite trans_C_ctx_M; rw; eauto] |
          rewrite inject_ctx_M; rewrite trans_C_ctx_M;
          rw; s; rewrite ID_refl; subst p']
      | _ =>
        exploit (PRE (PM M p')); s;
        first [solve [rewrite inject_ctx_M; rewrite trans_C_ctx_M;
    ...
    rewrite trans_C_inject. s. rrw.
    rewrite inject_assoc. s.
    ...
    ii; repeat des_hyp; des; clarify;
    repeat rewrite trans_C_inject in *; ss;
    ...
    ii; repeat des_hyp; des; clarify;
    repeat rewrite trans_C_inject in *; ss;
    repeat rewrite inject_assoc in *; ss; repeat rrw.
    split. split; eauto.
    ii. destruct p0; ss;
    first [rewrite inject_addr_x in *
      | rewrite inject_ctx_M in *];
...
Proof.
  ...
  des_hyp; des; clarify.
  all:match goal with
  | RR : Some ?v = read _ _ |- _ =>
    exists v; rewrite read_top; rewrite <- RR; eauto
  end.
Qed.
  ...
  | (t', v') :: tl' =>
    ...
    | Some t =>
      ...
      | Some (Closure e C) =>
        ...
        | None => Failed
        | Some (C, ϕ, tϕ) =>
          Changed t' v' t (Closure e C) ϕ tϕ
        end
      end end
    end
  end.
...
Definition trans_m_step_post `{Eq TT} `{Eq aTT} `{TotalOrder T} `{Eq aT}
  ...
  | Unchanged =>
    forall t' (IN : None <> read_fst ϕ t'),
      None = read m' t'
  | Changed t' (Closure e' C') t (Closure e C) ϕ_af tϕ_af =>
    Some t = read_fst ϕ t' /\ e' = e /\
    Some (Closure e' C') = read m' t' /\
    In (Closure e (trans_C α C)) (aread m (α t)) /\
...
Proof.
  ...
  - des_hyp; des; clarify.
    exploit IHp; eauto. intros [p' [VALp' INp']].
    exists (Pv e p').
    s. split; eauto. exists (Closure e C).
    rewrite read_top.
...
Proof.
  ...
  - des_hyp; des; clarify.
    exploit IHp; eauto. intros [p' [VALp' INp']].
    exists (Pv e p'). s. split; eauto.
    exists (Closure e C). eauto.
Qed.
...
Proof.
  ...
  - exploit IHp; eauto.
    ...
    destruct p'; ss; des; clarify.
    clarify.
    right. exists t'. exists e. exists C0.
  ...
  - des_hyp; des; clarify. right.
    ...
    intros REACH'; des; clarify;
    destruct REACH' as [p' [? ?]].
    exists t0. exists e. exists C.
    repeat (split; eauto). exists p'. eauto.
    exists t'. exists e0. exists C0.
    repeat (split; try assumption). right.
    exists (Pv e p').
    s. split; eauto.
    exists (Closure e C). eauto.
...
Definition equiv_fst_ϕ_m `{Eq T} `{Eq TT} (ϕ : list (T * TT)) m m' :=
  forall t p e C
  (READ : Some (Closure e C) = read m t)
  (VALp : valid_path (Ctx C) [] p),
  match read_fst ϕ t with
  | Some t' =>
    match pmap_fst ϕ p with
    | Some p' =>
      exists C', Some (Closure e C') = read m' t' /\
        valid_path (Ctx C') [] p'
    | None => False
    end 
  | None => False
...
Proof.
  ...
  assert (same ((t, Closure e0 C) :: m2 ++ remove_t m t) (m2 ++ m)) as SAME.
  {
    apply same_trans with (m' := m2 ++ ([(t, Closure e0 C)] ++ remove_t m t)).
    rewrite app_assoc.
    match goal with
    | |- context [?a :: ?b ++ ?c] =>
  ...
  exploit (IHn TT _ aTT _ T _ _ aT _ α' inv_α α φ φ' (ϕ' ++ ϕ) tϕ0 C2 C1 am ((t', Closure e0 C0) :: m1) ((t, Closure e0 C) :: m2) (remove_t m t)); eauto.
  ...
  - ii. split; intros REACH. apply tail_vpath in REACH.
    ...
    rewrite read_fst_top in *. repeat des_hyp; clarify.
    exploit (POST7 t0). rw. ii; clarify. intros [p [VALp INp]].
    apply paste_vpath with (t := t).
    apply reachable_disj with (m' := [(t, Closure e0 C)]); eauto.
    rewrite <- disj_reflect. ii; ss.
    des_hyp; clarify. rewrite eqb_eq in *; clarify.
    exists (Pv e0 p). s. split; eauto.
  ...
  - ii; ss. des_hyp; clarify.
    ...
    assert (read_fst (ϕ' ++ ϕ) t0 = Some t').
    {
      ...
      exploit (POST3 t0 t'). rewrite in_app_iff; eauto.
      ii. des. rrw. eauto.
    }
    ...
    assert (read_fst (ϕ' ++ ϕ) t0 = Some t1).
    {
      symmetry in HDES1.
      apply read_fst_in in HDES1.
      exploit (POST3 t0 t1). rewrite in_app_iff; eauto.
      ii. des. rrw. eauto.
    }
  ...
  - ii; ss. des_hyp; clarify.
    rewrite eqb_eq in *. clarify. exploit POST5; eauto.
    assert (read_snd (ϕ' ++ ϕ) t0 = Some t).
    {
      apply read_fst_in in POST.
      exploit (POST3 t t0). rewrite in_app_iff; eauto.
      ii. des. rrw. eauto.
    }
    rw. ii. des_hyp. rewrite t_refl. eexists. split; eauto.
    exploit PRE4; eauto.
    ...
    assert (read_snd (ϕ' ++ ϕ) t0 = Some t1).
    {
      ...
      exploit (POST3 t1 t0). rewrite in_app_iff; eauto.
      ii. des. rrw. eauto.
    }
    ...
    assert (eqb t t1 = false).
    {
      ...
      rewrite <- x2 in *. clarify. rewrite t_refl in *. clarify.
    }
    rw. eexists. split; eauto.
  - eauto.
  - eauto.
...
Proof.
  ...
  - rewrite <- disj_reflect. ii.
    ...
    destruct e as [ev Cv].
    symmetry in CASE.
    exploit (POSTm6 t0 Pnil ev Cv CASE); ss.
