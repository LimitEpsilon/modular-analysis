...
Lemma time_bound_project `{TotalOrder T} :
  forall s C t (BOUND : time_bound_C C t),
    match project C s with
    | None => True
    | Some Cs => time_bound_C Cs t
    end.
Proof.
  induction s; ii; ss;
  repeat des_goal; repeat des_hyp; clarify;
  ...
  try match goal with
  | BD : time_bound_C ?C ?t,
    RR : addr_x ?C ?x = Some _ |- _ =>
    pose proof (time_bound_addr C x t BD);
    rewrite RR in *; eauto
  | BD : time_bound_C ?C ?t,
    RR : ctx_M ?C ?M = Some _ |- _ =>
    pose proof (time_bound_ctx_M C M t BD);
    rewrite RR in *; eauto
  end;
  repeat match goal with
  | BD : time_bound_C ?C ?t,
    IH : forall C t, _ -> match project C ?s with _ => _ end,
    PROJ : project ?C ?s = _ |- _ =>
    specialize (IH C t BD); rewrite PROJ in *
  end; eauto.
Qed.

Lemma supp_inject `{TotalOrder T} :
  forall C1 C2 (t : T),
    supp_C (C1 [|C2|]) t <-> supp_C C1 t \/ supp_C C2 t.
Proof.
  induction C1; ii; ss; split; ii;
  try match goal with
  | IH : context [supp_C _ _ <-> _ \/ _] |- _ =>
    rewrite IH in *
  end; des; clarify; eauto.
Qed.
...
Proof.
  ...
  | BD : time_bound_C ?C ?t,
    ...
    pose proof (time_bound_ctx_M C M t BD) as HINT;
    rewrite RR in HINT
  | _ =>
    repeat match goal with
  ...
  | PROJ : project ?C ?s_M = _,
    ...
    pose proof (time_bound_project s_M C t BD);
    rewrite PROJ in *; solve_leb
  end.
  all:rewrite supp_inject in *; des; eauto 2.
  all:try match goal with
...
Ltac gen_time_bound T :=
  ...
  | E1 : @step T _ _ _ (Cf ?e ?C ?m ?t) (Rs ?V ?m' ?t'),
    ...
    E3 : {| (Cf ?e'' ?C'' ?m_up ?t_up) ~> (Rs ?V'' ?m''' ?t''') |} |- _ =>
    ...
    | _ : time_bound_ρ (Rs V m' t') |- _ =>
      ...
      | _ : time_bound_ρ (Cf e' C' m' t') |- _ =>
        ...
        | _ : time_bound_ρ (Rs V' m'' t'') |- _ =>
          let BD3 := fresh E3 in
          assert (time_bound_ρ (Cf e'' C'' m_up t_up)) as BD3;
          lazymatch goal with
