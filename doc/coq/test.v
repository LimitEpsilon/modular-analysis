...
Proof. 
  ...
  - unfold st_len in *. intros. destruct st. intros.
    inversion H as [ECASE [MCASE EQ]]. split.
    + remember (e_mem addr) as e_val.
      destruct e_val. destruct e. split.
      intros. eapply ECASE; eauto. split.
      intros. eapply MCASE; eauto. eapply ECASE; eauto. eauto.
    + remember (m_mem addr) as m_val.
      destruct m_val. destruct m0. split.
      intros. eapply ECASE; eauto. split.
      intros. eapply MCASE; eauto. eapply MCASE; eauto. eauto.
  - intros p C st p' C' st' INIT REACH.
    unfold st_len in *. destruct st, st'. intros.
    inversion INIT as [ECASEI [MCASEI EQI]].
    inversion REACH as [ECASER [MCASER EQR]]. split; intros; eauto.
  - intros. unfold st_len in *.
    destruct st, st_lam.
    inversion INIT as [ECASEI [MCASEI EQI]].
    inversion FN as [ECASEF [MCASEF EQF]].
    inversion ARG as [ECASEA [MCASEA EQA]].
    repeat split;
    intros; unfold update_m in *; try destruct (eq_p addr (t :: p_lam)); eauto.
    + inversion H; subst; eauto.
    + rewrite c_plugin_adds_level; simpl. nia.
  - intros. unfold st_len in *.
    destruct st.
    inversion INIT as [ECASEI [MCASEI EQI]].
    inversion EVx as [ECASEx [MCASEx EQx]].
    repeat split;
    intros; unfold update_m in *; try destruct (eq_p addr (t :: p)); eauto.
    + inversion H; subst; eauto.
    + rewrite c_plugin_adds_level; simpl. nia.
  - intros. unfold st_len in *.
    destruct st.
    inversion INIT as [ECASEI [MCASEI EQI]].
    inversion EVM as [ECASEM [MCASEM EQM]].
    repeat split;
    intros; unfold update_m in *; try destruct (eq_p addr (t :: p)); eauto.
    + inversion H; subst; eauto.
    + rewrite c_plugin_adds_level; simpl. nia.
...
Lemma preserve_st_len : forall e p' C' st' e'
        (REACH : <e| [] c_hole (ST empty_mem empty_mem 0) e ~> p' C' st' e' |e>),
        st_len 0 0 p' C' st'.
Proof.
  intros. unfold st_len in *.
  assert (INIT : st_len 0 0 [] c_hole (ST empty_mem empty_mem 0)).
  { unfold st_len in *; repeat split; intros; try inversion H. }
  specialize (ere_p_is_level 0 0 [] c_hole (ST empty_mem empty_mem 0) e
                             p' C' st' e'
                             INIT REACH).
  destruct st'. unfold st_len. eauto.
Qed.

Theorem level_is_len_p : forall e p' C' st' e'
        (REACH : <e| [] c_hole (ST empty_mem empty_mem 0) e ~> p' C' st' e' |e>),
        level C' = len_p p'.
Proof.
  pose proof preserve_st_len as H.
  intros. specialize (H e p' C' st' e' REACH).
  unfold st_len in H. destruct st'.
  destruct H as [mem_e [mem_m eq]].
  nia.
Qed.
