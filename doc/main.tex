...
\begin{definition}[Collecting semantics]
  ...
  \]
\end{definition}
Note that the above definition can be defined without qualms for situations when the $\ctx$ in $(e,\ctx)$ does not close $e$.
...
\[
  ...
  \begin{cases}
    ...
    (e,\inject{\ctx_1}{\ctx_2})                                    & r_2 = (e, \ctx_2)
  \end{cases}
\]
...
\begin{lemma}[Injection Preserves Evaluation]
  For all $\config\in\Config$, $\rightst\in\Right$, $\config\semarrow\rightst\Rightarrow\inject{\ctx}{\config}\semarrow\inject{\ctx}{\rightst}$.
\end{lemma}
\begin{proof}[Sketch]
...
There are no restrictions on $\tick$ and the states $(\ctx,\mem,t)$, thus a write to the address $t$ may overwrite an existing value that may be used for future computations.
...
\begin{lemma}[Operational Soundness]\label{lem:opsound}
  For all $\config\in\Config$ and $\rightst\in\Right$, if $\config\semarrow\rightst$ then $\A\alpha(\config)\A\semarrow\A\alpha(\rightst)$.
\end{lemma}
\begin{proof}[Sketch]
  Induction on $\semarrow$.
\end{proof}
...
Then if we define:
\[\A\Sigma\triangleq\A\Right+\A\semarrow\qquad\Abs{\Trace}\triangleq\pset(\A\Sigma)\A\Sigma\triangleq\A\Right+\A\semarrow\]
...
Also, we need to link $\tick$ well so that for all valid transitions $\config_2\semarrow\rightst_2$ under $\tick_2$, $\inject{s_1}{\config_2}\semarrow\inject{s_1}{\rightst_2}$ is also a valid transition under $\tick_+$.
...
Now we define injection between $s_1=(\ctx_1,\mem_1,t_1)\in\State_1$ and $r_2=(V_2,\mem_2,t_2)\in\Outcome_2$:
\[
  \begin{array}{cc}
...
As is expected, injecting $s_1$ into $r_2$ involves injecting $\ctx_1$ in every context in $r_2$ and merging the memories.
...
First, $\inject{s_1}{r_2}\in\Outcome_+$ with respect to the ordering $\le_+$.
Also, if we define $\inject{s_1}{\config_2}$ when $\config_2=(e,s_2)$ by $(e,\inject{s_1}{s_2})$, we can show that injection preserves valid transitions. That is,
\begin{lemma}[Injection Preserves Evaluation]
  For all $s_1\in\State_1$ and $\config_2\in\Config_2$,
  if $\config_2\semarrow\rightst_2$ under $\tick_2$, then $\inject{s_1}{\config_2}\semarrow\inject{s_1}{\rightst_2}$ under $\tick_+$.
\end{lemma}
\begin{proof}[Sketch]
...
\begin{definition}[Injection]
  ...
  \[
    S_1\rhd A_2\triangleq\{\inject{s_1}{\rightst_2}|s_1\in S_1,\rightst_2\in A_2\}\cup\{\inject{s_1}{\config_2}\semarrow\inject{s_1}{\rightst_2}|s_1\in S_1,\config_2\semarrow\rightst_2\in A_2\}
  \]
\end{definition}
\begin{definition}[Semantic Linking]
  For $S_1\subseteq\State_1$ and $A_2\subseteq\Sigma_2$, define:
...
\begin{lemma}[Injection Preserves Abstract Evaluation]
  For all $\A{s}_1\in\A\State_1$ and $\A\config_2\in\A\Config_2$, if $\A\config_2\A\semarrow\A\rightst_2$ under $\A\tick_2$,
  then $\inject{\A{s}_1}{\A\config_2}\A\semarrow\inject{\A{s}_1}{\A\rightst_2}$ under $\A\tick_+$.
\end{lemma}
\begin{proof}[Sketch]
...
\begin{definition}[Abstract Injection]
  ...
  \[
    \Abs{S}_1\Abs\rhd\Abs{A}_2\triangleq\{\inject{\A{s}_1}{\A\rightst_2}|\A{s}_1\in\Abs{S}_1,\A\rightst_2\in\Abs{A}_2\}\cup\{\inject{\A{s}_1}{\A\config_2}\A\semarrow\inject{\A{s}_1}{\A\rightst_2}|\A{s}_1\in\Abs{S}_1,\A\config_2\A\semarrow\A\rightst_2\in\Abs{A}_2\}
  \]
\end{definition}
...
\]
due to $\alpha_+(\inject{s_1}{r_2})=\inject{\alpha_1(s_1)}{\alpha_2(r_2)}$, the above theorem directly leads to overapproximation by:
\begin{align*}
