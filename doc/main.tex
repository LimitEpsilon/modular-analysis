...
\begin{figure}[h!]
  ...
  \[
    ...
    \begin{cases}
      ...
      \filter_1(C')                        & C=(x,t)\cons C'\wedge t.1=f_1     \\
  ...
  \[
    ...
    \begin{cases}
      []                                   & C=[]                              \\
      (x,t.2)\cons\filter_2(C')            & C=(x,t)\cons C'\wedge t.1=f_1     \\
...
Note that the linked time domain only uses timestamps of the form $(t_1,0_2)$ and $(f_1,t_2)$.
...
\[\Time_1\otimes\Time_2\triangleq\Time_1\times\{0_2\}\cup\{f_1\}\times\Time_2\]
  ...
  \begin{itemize}
    ...
    \item Define the ${\tick_{+}}({s_1})$ function as:
          \[
            ...
            \begin{cases}
              (\tick_1\:\filter_1(C,\mem,t,x,v), 0_2)                           & (\pi_1(t)\neq f_1) \\
...
\begin{definition}[Injection of a configuration : $\rhd$]
  ...
  \[
    ...
    \lambda (t_1,t_2).
    \begin{cases}
      m_1(t_1)               & t_1\in\mathsf{dom}(m_1)\wedge t_2=0_2 \\
      \inject{C_1}{m_2}(t_2) & t_1=f_1\wedge t_2\in\mathsf{dom}(m_2)
    \end{cases}
    \qquad
    s\rhd r\triangleq
...
\begin{definition}[Concrete linking operator]
  \begin{align*}
    \Link\:e_1\:e_2\:s & \triangleq(\sembracket{e_1}(s),0_2)\cup\LL\:(\EE\:e_1\:s)\:e_2\cup[(\link{e_1}{e_2},(s,0_2))\mapsto\mathsf{F}\:(\EE\:e_1\:s)\:e_2]
  \end{align*}
\end{definition}
...
\begin{lem}[Linked abstraction]
  ...
  \begin{itemize}
    \item Let all timestamps in $C_1,\mem_1$ be less than $f_1$.
...
The fact that the analysis $\A\LL_n$ can be computed without actually computing the final times $f_n$ is why this analysis can be called separate.
