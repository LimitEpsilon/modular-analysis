...
\begin{definition}[Versions of $\A\semarrow$, $\Abs\Step$]$\:$
  ...
  \begin{enumerate}
    \item $\A\semarrow_1$, $\A\semarrow_2$, $\A\semarrow_{\semlink}$ are versions of $\A\semarrow$ that each use $(\A\Time_1,\A\tick_1)$, $(\A\Time_2,\A\tick_2)$, $(\A\Time_{\semlink},\A\tick_{\semlink})$.
    \item $\Abs\Step_1$, $\Abs\Step_2$, $\Abs\Step_{\semlink}$ are versions of $\Abs\Step$ that each use $\A\semarrow_1$, $\A\semarrow_2$, $\A\semarrow_{\semlink}$.
  \end{enumerate}
\end{definition}
...
\]
due to $\alpha_{\semlink}(\inject{s_1}{r_2})=\inject{\alpha_1(s_1)}{\alpha_2(r_2)}$, the above theorem directly leads to overapproximation by:
\begin{align*}
