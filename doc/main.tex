...
\begin{definition}[Weak update]
  Given $\A{\sigma}\in\AMem{\ATime}$, $\A{t}\in\ATime$, $\A{v}\in\Value{\ATime}$, we define $\A{\sigma}[\A{t}\A{\mapsto}\A{v}]$ as:
  \[
    \A{\sigma}[\A{t}\A{\mapsto}\A{v}](\A{t'})\triangleq
    \begin{cases}
      \A{\sigma}(\A{t})\cup\{\A{v}\} & (\A{t'}=\A{t})     \\
      \A{\sigma}(\A{t'})             & (\text{otherwise})
    \end{cases}
  \]
\end{definition}
...
\end{definition}
In our semantics, $\A{\tick}\:\A{C}\:\A{\sigma}\:\A{t}\:x\:\A{v}$ is performed when $\A{v}$ is bound to $x$ under configuration $(\A{C},\A{\sigma},\A{t})$.
