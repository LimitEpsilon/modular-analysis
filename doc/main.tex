...
\begin{definition}[Linking of abstraction]
  ...
  \begin{itemize}
    ...
    \item Define the abstractions $\alpha^{<}_{+},\alpha^{>}_{+}:\Time_1\times\Time_2\rightarrow\ATime_1+\ATime_2$ as:
          \[
            \alpha^{<}_{+}\triangleq
            \begin{cases}
              \alpha_1(\pi_1(t)) & \pi_1(t)<f_1    \\
              \alpha_2(\pi_2(t)) & \pi_1(t)\ge f_1
            \end{cases}
            \qquad
            \alpha^{>}_{+}\triangleq
            \begin{cases}
              \alpha_1(\pi_1(t)) & \pi_1(t)\le f_1 \\
              \alpha_2(\pi_2(t)) & \pi_1(t)>f_1
            \end{cases}
          \]
  \end{itemize}
  We define the linked abstraction function $\alpha_+(R)\triangleq\alpha^{<}_+(R)\cup\alpha^{>}_+(R)$.
\end{definition}
