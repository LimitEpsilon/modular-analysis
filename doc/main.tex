...
\RenewDocumentCommand\SimpleBNFDefEq{}{\ensuremath{\rightarrow}}
...
\begin{bnfgrammar}
  $t$ : $\mathrm{Time}$ \in \Time
  ;;
  $C$ : $\mathrm{Context}$ \in \Ctx{\Time}
  ;;
  $v$ : $\mathrm{Result\:of\:expressions}$ \in \Value{\Time} \triangleq \Expr\times\Ctx{\Time}
  ;;
  $V$ : $\mathrm{Result\:of\:expressions/modules}$ \in \Value{\Time}+\Ctx{\Time}
  ;;
  $\mem$ : $\mathrm{Memory}$ \in \Mem{\Time} \triangleq \fin{\Time}{\Value{\Time}}
  ;;
  $s$ : $\mathrm{Configuration}$ \in \Config{\Time} \triangleq \Ctx{\Time}\times\Mem{\Time}\times\Time
  ;;
  $r$ : $\mathrm{Result}$ \in \Result{\Time} \triangleq (\Value{\Time}+\Ctx{\Time})\times\Mem{\Time}\times\Time
  ;;
  $C$ : $\mathrm{Context}$ ::= [] : hole
...
\begin{definition}[Linking operator]
  \begin{align*}
    ...
    \Link\:e_1\:e_2\:s  & \triangleq\sembracket{e_1}(s)\cup\LL\:(\EE\:e_1\:s)\:e_2\cup[(\link{e_1}{e_2},s)\mapsto\LL^{\circ}\:(\EE\:e_1\:s)\:e_2] & (\text{Linking of }e_1\text{ and }e_2\text{ under }s)
  \end{align*}
\end{definition}
