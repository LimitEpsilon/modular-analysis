...
\begin{defn}[Equivalent Concrete States]
  ...
  \begin{enumerate}
    \item $\forall p\in\Path:$ If $\valid(C,\mem,p)$, $\valid(C',\mem',\varphi(p))$ and $p=\varphi'(\varphi(p))$.
    \item $\forall p'\in\Path':$ If $\valid(C',\mem',p')$, $\valid(C,\mem,\varphi'(p'))$ and $p'=\varphi(\varphi'(p'))$.
  \end{enumerate}
\end{defn}
\begin{defn}[Weakly Equivalent Abstract States]\label{def:weakequiv}
  ...
  \begin{enumerate}
    \item $\forall\A{p}\in\A\Path:$ If $\A\valid(\A{C},\A\mem,\A{p})$, $\A\valid(\A{C'},\A{\mem'},\A\varphi(\A{p}))$ and $\A{p}=\A\varphi'(\A\varphi(\A{p}))$.
    \item $\forall\A{p'}\in\A{\Path'}:$ If $\A\valid(\A{C'},\A{\mem'},\A{p'})$, $\valid(\A{C},\A{\mem},\A\varphi'(\A{p'}))$ and $\A{p'}=\A\varphi(\A\varphi'(\A{p'}))$.
  \end{enumerate}
\end{defn}
